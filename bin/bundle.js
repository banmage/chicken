...
(function (CCR) {
    ...
    class Chicken extends Laya.Sprite {
        constructor() {
            ...
            this.isOnBank = true;
            // 确保小鸡初始位置在底部河岸上
            this.pos(CCR.GAME_WIDTH / 2 - CCR.CHICKEN_WIDTH / 2, CCR.BOTTOM_BANK_Y - CCR.CHICKEN_HEIGHT);
            this.createView();
        }
        createView() {
            this.graphics.clear();
        ...
        respawnToStart() {
            ...
            this.y = CCR.GAME_HEIGHT - CCR.CHICKEN_HEIGHT - 20;
            this.isOnBank = true;
            this.safeStayTimer = 0;
        }
        destroy() {
    ...
    class MenuUI extends Laya.Sprite {
        constructor() {
            ...
            this.menuChickenY = 290;
            this.createUI();
        }
        createUI() {
            // 深色渐变背景
            this.graphics.drawRect(0, 0, CCR.GAME_WIDTH, CCR.GAME_HEIGHT, '#1a1a2e');
            // 顶部装饰条
            this.graphics.drawRect(0, 0, CCR.GAME_WIDTH, 100, '#16213e');
            // 底部装饰条
            this.graphics.drawRect(0, CCR.GAME_HEIGHT - 100, CCR.GAME_WIDTH, 100, '#16213e');
            // 左侧装饰线
            this.graphics.drawRect(0, 80, 6, CCR.GAME_HEIGHT - 160, '#e94560');
            // 右侧装饰线
            this.graphics.drawRect(CCR.GAME_WIDTH - 6, 80, 6, CCR.GAME_HEIGHT - 160, '#e94560');
            // 标题背景
            ...
            this.menuChicken.pos(CCR.GAME_WIDTH / 2, 290);
            ...
            version.fontSize = 12;
            version.color = '#555';
            version.pos(CCR.GAME_WIDTH - 50, CCR.GAME_HEIGHT - 25);
        ...
        animateMenuChicken() {
            ...
            this.menuChicken.x = CCR.GAME_WIDTH / 2 + Math.sin(this.menuChickenAngle) * 60;
    ...
    class GameMain extends Laya.Sprite {
        ...
        createBackground() {
            ...
            // 计算实际河道数量
            const actualLanes = Math.max(1, Math.floor((riverHeight - CCR.LANE_GAP) / CCR.LANE_GAP));
            // 使用两种深浅相近的蓝色交替
            const colors = ['#1E90FF', '#2894E8'];
            for (let laneIndex = 0; laneIndex < actualLanes; laneIndex++) {
                const laneStartY = riverStartY + laneIndex * CCR.LANE_GAP;
                const colorIndex = laneIndex % colors.length;
                this.graphics.drawRect(0, laneStartY, CCR.GAME_WIDTH, CCR.LANE_GAP, colors[colorIndex]);
            }
        }
        createBanks() {
            // 底部赭红色河岸（出发区域）
        ...
        createLanes(config) {
            ...
            for (let laneIndex = 0; laneIndex < actualLanes; laneIndex++) {
                ...
                for (let i = 0; i < boatCount; i++) {
                    ...
                    // 创建船，确保船在河道中央（Y方向）
                    const boatY = laneStartY + CCR.LANE_GAP / 2 - CCR.BOAT_HEIGHT / 2;
                ...
                if (!difficulty.shouldHideObstacles()) {
                    for (let i = 0; i < Math.min(config.obstacles, 3); i++) {
                        ...
                        const obstacle = new Obstacle(x, laneStartY + CCR.LANE_GAP / 2 - CCR.OBSTACLE_HEIGHT / 2, config.boatSpeed * 0.8, direction);
        ...
        handleJump(direction) {
            ...
            if (currentY >= CCR.BOTTOM_BANK_Y - CCR.CHICKEN_HEIGHT) {
                if (direction === -1) {
                    // 从底部河岸向上跳到第一条河道
                    targetY = CCR.BOTTOM_BANK_Y - CCR.BANK_HEIGHT - CCR.LANE_GAP + CCR.BOAT_HEIGHT / 2 - CCR.CHICKEN_HEIGHT / 2;
                }
                else {
                    // 向下跳，保持在底部河岸
                    targetY = CCR.BOTTOM_BANK_Y - CCR.CHICKEN_HEIGHT;
                    DifficultyManager.getInstance().recordJump(false);
            ...
            // 如果小鸡已经在顶部河岸或以上
                ...
                if (direction === 1) {
                    // 从顶部河岸向下跳到最上面的河道
                    targetY = CCR.TOP_BANK_Y + CCR.BANK_HEIGHT + CCR.LANE_GAP + CCR.BOAT_HEIGHT / 2 - CCR.CHICKEN_HEIGHT / 2;
                }
                else {
                    // 向上跳，已经在顶部，尝试跳到岸上
